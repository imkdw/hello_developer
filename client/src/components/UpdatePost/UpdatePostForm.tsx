...
const UpdatePostForm = () => {
  ...
  const [postData, setPostData] = useState<AddPostData>({
    ...
    tags: [
      { name: postDetailData.tags[0] ? postDetailData.tags[0].name : "" },
      { name: postDetailData.tags[1] ? postDetailData.tags[1].name : "" },
      { name: postDetailData.tags[2] ? postDetailData.tags[2].name : "" },
    ],
  ...
  const submitHandler = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const status = await PostService.update(body, currentPostId, loggedInUser.accessToken);
      if (status === 200) {
        alert("게시글 업데이트 성공");
        navigator(-1);
      }
  ...
  return (
    <StyledUpdatePostForm onSubmit={submitHandler}>
      <Wrapper>
        ...
        <FormControl>
          ...
          <InputWrapper>
            ...
            <Input
              ...
              name="tag3"
              onChange={changeTags}
              value={postData.tags[2].name}
            />
          </InputWrapper>
        </FormControl>
        <FormControl>
          <Label>내용</Label>
          <TextEditor onChange={changeContent} value={postData.content} />
        </FormControl>
        <Buttons>
